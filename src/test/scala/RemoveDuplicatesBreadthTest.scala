...
// testy do zadania 3 z wyszukiwaniem wszerz (3 punkty)
class RemoveDuplicatesBreadthTest extends FunSuite {
  ...
  test("basicTest1") {
    ...
    val (result1, result2) = Lista4.removeDuplicatesBreadth(mockTree1, mockTree2)
  ...
  test("allEqualElements") {
    ...
    assert(Lista4.removeDuplicatesBreadth(mockTree, mockTree) == (Empty, Empty))
  }
  ...
  test("allDifferentElements") {
    ...
    assert(Lista4.removeDuplicatesBreadth(mockTree1, mockTree2) == (mockTree1, mockTree2))
  }

  test("differentDepths") {
    assertThrows[IllegalArgumentException](
      Lista4.removeDuplicatesBreadth(

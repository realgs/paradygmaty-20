package Lista4
import Lista4.Lista4Trees.{Empty, Node, createTree, inorder, subtractTrees}
...
class Lista4TreesTest extends FunSuite {
  ...
  test("Lista 4 zadanie 2") {
    val tree1 = createTree(3, 1, 50)
    val tree2 = createTree(3, 1, 50)
    val result = inorder(tree1).zip(inorder(tree2)).map(n => n._1 - n._2)
    assert(inorder(subtractTrees(tree1, tree2)) == result)
  }
  ...
  test("Lista 4 zadanie 3 BFS") {
    assert(Lista4Trees.ifSubTreesEqualsBFS(Node(8, Node(10, Node(1, Empty, Empty), Node(2, Empty, Empty)), Node(7, Node(3, Empty, Empty), Node(15, Empty, Empty))), Node(8, Node(10, Node(1, Empty, Empty), Node(2, Empty, Empty)), Node(7, Node(3, Empty, Empty), Node(15, Empty, Empty)))))
    assert(Lista4Trees.ifSubTreesEqualsBFS(Node(4, Node(2, Empty, Empty), Node(-1, Node(5, Empty, Empty), Node(5, Empty, Empty))), Node(4, Node(2, Empty, Empty), Node(-1, Node(5, Empty, Empty), Node(5, Empty, Empty)))))
    assert(!Lista4Trees.ifSubTreesEqualsBFS(Node(2, Node(3, Node(2, Empty, Empty), Node(4, Empty, Empty)), Node(3, Node(3, Empty, Empty), Node(2, Empty, Empty))), Node(2, Node(3, Empty, Empty), Node(-1, Node(3, Empty, Empty), Node(2, Empty, Empty)))))

...
object ParallelProgramming {
  ...
  def longestCommonPrefix(words: Array[String]): String = {
    @tailrec
    def longestCommonPrefixHelper(first: String, second: String, result: String): String = (first, second) match {
      case ("", _) | (_, "") => result
      case _ => if (first(0) == second(0)) longestCommonPrefixHelper(first.substring(1), second.substring(1), result + first(0)) else result
    }
    ...
    def commonPrefix(startingIndex: Int, endingIndex: Int): String = {
      if (startingIndex < endingIndex) {
        if (endingIndex - startingIndex == words.length - 1) {
          ...
          longestCommonPrefixHelper(Await.result(futureLeft, Duration.Inf), Await.result(futureRight, Duration.Inf), "")
        }
        else {
          ...
          longestCommonPrefixHelper(leftLongestPrefix, rightLongestPrefix, "")
        }
      }
